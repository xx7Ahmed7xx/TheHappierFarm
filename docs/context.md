...
**Locales**: English + Arabic (`client/src/i18n/`). Catalog display names from SQL (`DisplayNameEn` / `DisplayNameAr` on crops, animals, factories, decorations, resources). HUD/shop/modals use `catalogNames.ts` + `recipeFormat.ts` (no raw English codes in Arabic UI).

**Audio**: Web Audio SFX + looping MP3 BGM (`bgm-menu.mp3`, `bgm-farm.mp3`) with procedural fallback (`audio.ts`, `bgmPlayer.ts`). Menu vs farm tracks differ. Global **♪** toggles music (`ClientPresentation` unrelated).
